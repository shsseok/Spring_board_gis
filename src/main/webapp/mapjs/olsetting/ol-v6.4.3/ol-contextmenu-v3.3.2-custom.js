...
(function (global, factory) {
  ...
  Internal.prototype.openMenu = function openMenu (pixel, coordinate) {
    if (typeof(this.Base.options.openFn) == 'function') this.Base.options.openFn(this.items);
  ...
  Internal.prototype.closeMenu = function closeMenu () {
    if (typeof(this.Base.options.closeFn) == 'function') this.Base.options.closeFn(this.items);
    this.opened = false;
  ...
  Html.prototype.addMenuEntry = function addMenuEntry (item) {
    ...
    if (item.items && Array.isArray(item.items)) {
      ...
      /* custom start */
      item.classname = (item.classname.length != 0 ? CSS_VARS.submenu + " " + item.classname : item.classname);
      var li = this.generateHtmlAndPublish(this.container, item);
      if(window.jQuery && item.classname.contains("contextMenuItem")){
    	  jQuery(li).data("contextItem", item);
      }
      /* Custom end */
  ...
  Html.prototype.generateHtmlAndPublish = function generateHtmlAndPublish (parent, item, submenu) {
    ...
    if (typeof item === 'string' && item.trim() === '-') {
      ...
      var _Base = this.Base;
      if (typeof(this.Base.options.mouseoverFn) == 'function')  {
        element.addEventListener(
          'mouseover',
          function(evt) {
            evt.preventDefault();
            _Base.options.mouseoverFn(item);
          }
        );

      }
      if (typeof(this.Base.options.mouseoutFn) == 'function')  {
        element.addEventListener(
          'mouseout',
          function(evt) {
            evt.preventDefault();
            _Base.options.mouseoutFn(item);
          }
        );
      }
