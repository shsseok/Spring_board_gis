...
public class BoardController {
	...
	@GetMapping(value="/boardList/Select.do", produces=MediaType.APPLICATION_JSON_VALUE) //-> param 페이지 넘버가 넘어오기 때문에 방대한 데이터가 아님
	@ResponseBody
	...
	@PostMapping(value="/insertBoardAction.do",consumes="multipart/form-data") //->multipart/form-data 이 Contents-type으로만 받겠다는 의미
	public String insertBoardAction(@Valid @ModelAttribute BoardVo boardVo, BindingResult result,
			@RequestParam("boardFiles") List<MultipartFile> files, RedirectAttributes redirectAttributes) throws Exception {
	...
	public String getBoardById(Model model,@RequestParam(defaultValue="no") String viewSet,@RequestParam int boardId) {	
	...
	@ResponseBody
	public Map<String, Integer> updateBoard(@RequestBody BoardVo boardVo) {
		Map<String, Integer> response = new HashMap<>();
		int isSuccess = boardService.boardUpdate(boardVo);
		if (isSuccess == 1) {
	...
	@ResponseBody
	public Map<String, Integer> deleteBoard(@RequestBody BoardVo boardVo) {
		Map<String, Integer> response = new HashMap<>();
		int isSuccess = boardService.boardDelete(boardVo);
		if (isSuccess == 1) {
			response.put("result", 1);
			return response;
		} else {
